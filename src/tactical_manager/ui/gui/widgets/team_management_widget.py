...
class TeamManagementWidget(QWidget):
    def __init__(self, club: Club) -> None:
        ...
        self.training_page = PlaceholderPage(
            "Training",
            "This page will later control player development, workload, recovery, and focus areas."
        )
        self.transfers_page = PlaceholderPage(
            "Transfers",
            "This page will later show available players, scouting, transfer targets, and contracts."
        )

        self.pages.addWidget(self.squad_page)
        self.pages.addWidget(self.lineup_page)
        self.pages.addWidget(self.tactics_page)
        self.pages.addWidget(self.training_page)
        self.pages.addWidget(self.transfers_page)

        self._add_menu_button(menu_layout, "Squad", 0)
        self._add_menu_button(menu_layout, "Lineup", 1)
        self._add_menu_button(menu_layout, "Tactics", 2)
        self._add_menu_button(menu_layout, "Training", 3)
        self._add_menu_button(menu_layout, "Transfers", 4)

        menu_layout.addStretch()

        outer_layout.addWidget(self.menu_panel)
        outer_layout.addWidget(self.pages, 1)

        self.set_page(0)

    def _add_menu_button(self, layout: QVBoxLayout, text: str, page_index: int) -> None:
        button = QPushButton(text)
        button.setCursor(Qt.PointingHandCursor)
        button.setMinimumHeight(42)
        button.clicked.connect(lambda checked=False, idx=page_index: self.set_page(idx))
        ...
        for i, button in enumerate(self.menu_buttons):
            if i == index:
                button.setStyleSheet("""
                    ...
                    QPushButton:hover {
                        background-color: rgba(90, 130, 190, 200);
                    }
                """)
            else:
                button.setStyleSheet("""
